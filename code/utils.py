...
class ReFlow:
    ...
    def train(self, mesh, data, train_seq, test_seq, lr, iters, result_path, wr_img=True, wr_ckp=True, ow_reso=None):
        ...
        for i in range(iters + 1):
            ...
            if i % 10000 == 0 and wr_img:  # train
                ...
                run_list = ['loss:0', 'lrn_t:0', 'tex_t:0', 'ref_t:0', 'dif_t_fin:0', 'dif_t:0', 'lrn_dif:0', 'dif_ref:0', self.test,
                            'flow_dir:0', 'dif_t_bld:0', 'ref_t_bld_tex:0', 'dif_w:0', 'ref_w:0', 'conf:0', 'weight:0', 
                            # 'warp1:0', 'warp2:0',
                            # 'curv:0'
                            'r:0', 'd:0', 'p:0'
                            ]
                test_loss, lrn, tex, ref, diftfin, dift, dif, difr, tst, fd, difb, refb, difw, refw, cf, wt, r, d, p = tf.get_default_session().run(run_list, feed)
                ...
                write_image(f'{result_path}/{i}_test_refb.png', refb[-1])
                # write_image(f'{result_path}/{i}_train_lrnb.png', lrnb[-1])
                j = 2
                write_image(f'{result_path}/{i}_test_{j}fd.png', fd[j])
                write_image(f'{result_path}/{i}_test_{j}cf.png', cf[j])
                # write_image(f'{result_path}/{i}_test_{j}w1.png', w1[j])
                # write_image(f'{result_path}/{i}_test_{j}w2.png', w2[j])
                write_image(f'{result_path}/{i}_test_{j}wt.png', wt[j])
                write_image(f'{result_path}/{i}_test_{j}dift.png', np.abs(dift[j]))
                write_image(f'{result_path}/{i}_test_{j}difr.png', np.abs(difr[j]))
                write_image(f'{result_path}/{i}_test_{j}difw.png', np.abs(difw[j]))

                write_image(f'{result_path}/{i}_test_dif.png', np.abs(dif[-1]))
                write_image(f'{result_path}/{i}_test_difr.png', np.abs(difr[-1]))
                write_image(f'{result_path}/{i}_test_difl.png', np.abs(diftfin[-1]))
                write_image(f'{result_path}/{i}_test_difb.png', np.abs(difb[-1]))
                

                # write_image(f'{result_path}/{i}_test_r.png', r[-1])
                # write_image(f'{result_path}/{i}_test_d.png', d[-1])
                # write_image(f'{result_path}/{i}_test_p.png', p[-1])

                # write_image(f'{result_path}/{i}_test_{0}fd.png', fd[0])
                # write_image(f'{result_path}/{i}_test_{1}fd.png', fd[1])
                # write_image(f'{result_path}/{i}_test_{2}fd.png', fd[2])
                # write_image(f'{result_path}/{i}_test_{3}fd.png', fd[3])
                # exit()
    ...
    def render(self, mesh, data, ckp_path, ckp_idx, train_seq, render_seq, result_path, inter, wr_img, adj_n, ow_reso=None):
        ...
        for i in render_seq:
            ...
            # dmin.append(np.min(d_t))
            # d_t *= m_t
            # dmax.append(np.max(d_t))
...
class DNR:
    ...
    def ras_graph(self, mesh, resolution, tex, w2p_in, view_pos_in):
        # geometry
        world_pos = mesh['v']
        vert_idx = mesh['fv']
        normal = mesh['vn']
        ...
        return tex_feat, inter_view_dir_unit, mask
    ...
    def fuse_net(self, feat, reuse=None):
        ...
        layers.append(conv_down(layers[-1], 64, activation=tf.nn.leaky_relu, regularizer=reg, name='rg0', reuse=reuse))
        layers.append(conv_down(instance_norm(layers[-1]), 128, activation=tf.nn.leaky_relu, regularizer=reg, name='rg1', reuse=reuse))
        layers.append(conv_down(instance_norm(layers[-1]), 256, activation=tf.nn.leaky_relu, regularizer=reg, name='rg2', reuse=reuse))
        layers.append(conv_down(instance_norm(layers[-1]), 512, activation=tf.nn.leaky_relu, regularizer=reg, name='rg3', reuse=reuse))
        layers.append(conv_down(instance_norm(layers[-1]), 512, activation=tf.nn.leaky_relu, regularizer=reg, name='rg10', reuse=reuse))

        layers.append(conv_up(instance_norm(layers[-1]), 512, activation=tf.nn.leaky_relu, regularizer=reg, name='rg11', reuse=reuse))
        layers.append(conv_up(instance_norm(layers[-1] + layers[4]), 256, activation=tf.nn.leaky_relu, regularizer=reg, name='rg4', reuse=reuse))
        layers.append(conv_up(instance_norm(layers[-1] + layers[3]), 128, activation=tf.nn.leaky_relu, regularizer=reg, name='rg5', reuse=reuse))
        layers.append(conv_up(instance_norm(layers[-1] + layers[2]), 64, activation=tf.nn.leaky_relu, regularizer=reg, name='rg6', reuse=reuse))
        # layers.append(instance_norm(conv_up(layers[-1] + layers[1], feat_len, regularizer=reg, name='rg7', reuse=reuse)))
        # layers.append(instance_norm(conv_up(layers[-1] + layers[0], feat_len, 4, 1, regularizer=reg, name='rg8', reuse=reuse)))
        layers.append(conv_up(layers[-1] + layers[1], 3, 4, 2, activation=tf.nn.tanh, regularizer=None, name='rg9', reuse=reuse))
    ...
    def construct_graph(self, mesh, resolution):
        ...
        tex = tf.Variable(tf.constant(0, tf.float32, [1, 1024, 1024, 16]), name='tex')
        ...
        ras = self.ras_graph(mesh, resolution, tex, w2p_in, view_pos_in)
        t, v, mask_t = [tf.pad(i[:, y0:y1, x0:x1], pad) for i in ras]

        # t = tf.concat([tf.nn.relu(t[..., :3]), t[..., 3:12], t[..., 12:]], axis=-1)
        tex_t = tf.multiply(t[..., :3], 1, name='tex_t')

        ref_in = tf.placeholder(tf.float32, [None, resolution[0], resolution[1], 3], name='ref_in')
        ref_in = tf.pad(ref_in[:, y0:y1, x0:x1], pad)
        ref_t = tf.multiply(ref_in[-1:], mask_t, name='ref_t')

        v_brd = tf.broadcast_to(view_pos_in[:, tf.newaxis], tf.concat([tf.shape(view_pos_in)[:1], tf.shape(ref_t)[1:3], view_pos_in.shape[-1:]], axis=-1))
        sh_view = sh_basis(v_brd, o=0)
        sh_tex = tf.concat([t[..., :3], t[..., 3:12] * sh_view, t[..., 12:]], axis=-1)

        lrn_t = tf.multiply(self.fuse_net(sh_tex, reuse=None), mask_t, name='lrn_t')
        ...
        # vgg_pix = tf.reduce_mean(loss_vgg(lrn_t, ref_t, mask_t))


        l1_tex = tf.reduce_mean(loss_l1(tex_t, ref_t, mask_t))


        loss_reg = tf.losses.get_regularization_loss()
        loss_opt = 1 * l1_pix + 1 * l1_tex #+ loss_reg

        self.test = [l1_pix, l1_tex]
    ...
    def train(self, mesh, data, train_seq, test_seq, lr, iters, result_path, wr_img=True, wr_ckp=True, ow_reso=None):
        ...
        for i in range(iters + 1):
            batch = 1
            sample_idx = [random.randint(0, len(train_seq) - 1) for _ in range(batch)]
            sample_seq = [copy.deepcopy(train_seq[s]) for s in sample_idx]
            ...
            if i % 1000 == 0 and wr_img:  # train
                feed = {
                    'w2p_in:0': w2p_seq[-1:],
                    'view_pos_in:0': view_pos_seq[-1:],
                    'ref_in:0': ref_seq[-1:],
                    'crop_hw:0': resolution,
                }
                run_list = ['loss:0', 'lrn_t:0', 'tex_t:0', 'ref_t:0', 'lrn_dif:0']

                test_loss, lrn, tex, ref, dif = tf.get_default_session().run(run_list, feed)

                write_image(f'{result_path}/{i}_train_lrn.png', lrn[-1])
                write_image(f'{result_path}/{i}_train_tex.png', tex[-1])
                write_image(f'{result_path}/{i}_train_ref.png', ref[-1])
                write_image(f'{result_path}/{i}_train_dif.png', np.abs(dif[-1]))
                

                # test
                sample_idx = random.randint(0, len(test_seq) - 1)
                sample_seq = [test_seq[sample_idx]]
                ...
                run_list = ['loss:0', 'lrn_t:0', 'tex_t:0', 'ref_t:0', 'lrn_dif:0', self.test]

                test_loss, lrn, tex, ref, dif, tst = tf.get_default_session().run(run_list, feed)
    ...
    def render(self, mesh, data, ckp_path, ckp_idx, train_seq, render_seq, result_path, inter, wr_img, adj_n=None, ow_reso=None):
        if ow_reso:
            resolution = ow_reso
        else:
            resolution = np.array([data[0][1]['height'], data[0][1]['width']])
        ...
        _, render_adj = make_adj(10, train_seq, render_seq)
        render_grp = np.concatenate([render_adj, np.reshape(render_seq, [-1, 1])], axis=-1)
        ...
        for i in render_seq:
            ...
            loss, lrn, ref, tex = tf.get_default_session().run(['loss:0', 'lrn_t:0', 'ref_t:0', 'tex_t:0'], feed)

            time_list.append(time.time() - t)
            loss_list.append(loss)

            write_image(f'{result_path}/lrn/{i}_lrn.png', lrn[-1])
            # write_image(f'{result_path}/tex/{i}_tex.png', tex[-1])
            if not inter:
                write_image(f'{result_path}/ref/{i}_ref.png', ref[-1])
...
class NVDR:
    ...
    def ras_graph(self, mesh, resolution, tex, env, w2p_in, view_pos_in):
        # geometry
        world_pos = mesh['v']
        vert_idx = mesh['fv']
        normal = mesh['vn']
        ...
        tex_feat = dr.texture(tex, inter_uv, filter_mode='linear', boundary_mode='wrap', tex_const=False, max_mip_level=None)
        tex_feat *= mask
        env_map = dr.texture(env, reflect_dir_unit, filter_mode='linear', boundary_mode='cube', tex_const=False, max_mip_level=None)
        env_map *= mask
        ...
        return tex_feat, env_map, inter_normal_unit, inter_view_dir_unit, reflect_dir_unit, mask
    ...
    def construct_graph(self, mesh, resolution):
        ...
        tex = tf.Variable(tf.constant(0, tf.float32, [1, 1024, 1024, 6]), name='tex')
        env = tf.Variable(tf.constant(0, tf.float32, [1, 6, 1024, 1024, 6]), name='env')
        ...
        ras = self.ras_graph(mesh, resolution, tex, env, w2p_in, view_pos_in)
        t, e, n, v, r, mask_t = [tf.pad(i[:, y0:y1, x0:x1], pad) for i in ras]
        
        tex_c = tf.multiply(t[..., :3], 1, name='tex_c')
        kd = t[..., 3:4]
        ks = t[..., 4:5]
        ep = t[..., 5:]
        ld = e[..., :3]
        ...
        # phong shadin
        a1 = tf.clip_by_value(tf.reduce_sum(v * n, axis=-1, keep_dims=True), 0, 1)
        d = a1 * kd * ld
        a2 = tf.clip_by_value(tf.reduce_sum(v * r, axis=-1, keep_dims=True), 0, 1)
        s = a2**ep * ks * ls
        lrn_t = tf.multiply(tex_c + d + s, mask_t, name='lrn_t')
        ...
        # mse_pix = loss_mse(lrn_t, ref_t, mask_t)
        mse_pix = loss_mse(tex_t, ref_t, mask_t)
        l1_pix = tf.reduce_mean(loss_l1(lrn_t, ref_t, mask_t))
    ...
    def train(self, mesh, data, train_seq, test_seq, lr, iters, result_path, wr_img=True, wr_ckp=True, ow_reso=None):
        ...
        for i in range(iters + 1):
            ...
            if i % 1000 == 0 and wr_img:  # train
                ...
                run_list = ['loss:0', 'lrn_t:0', 'tex:0', 'ref_t:0', 'lrn_dif:0', 'tex_c:0']

                test_loss, lrn, tex, ref, dif, tex_c = tf.get_default_session().run(run_list, feed)

                # write_image(f'{result_path}/{i}_train_lrn.png', lrn[-1])
                write_image(f'{result_path}/{i}_train_tex.png', tex[-1,...,:3])
                write_image(f'{result_path}/{i}_train_lrn.png', tex_c[-1,...,:3])
                # write_image(f'{result_path}/{i}_train_ref.png', ref[-1])
                # write_image(f'{result_path}/{i}_train_dif.png', np.abs(dif[-1]))
    ...
    def render(self, mesh, data, ckp_path, ckp_idx, train_seq, render_seq, result_path, inter, wr_img, ow_reso=None):
        if ow_reso:
            resolution = ow_reso
        else:
            resolution = np.array([data[0][1]['height'], data[0][1]['width']])
